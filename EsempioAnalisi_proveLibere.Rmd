...
# Modelli di previsione sinergici

```{r}

X0=X
X0$prev_pos=predict(modlm_pos,newdata = X)
X0$prev_neg=predict(modlm_neg,newdata = X)
X0$prev_ironic=predict(modlm_ironic,newdata = X)

contgg=grep("Conteggi\\.",names(X0))
names(X0)[contgg]=paste("simbolo",sep="",1:length(contgg))
...
library(rpart)
modrp=rpart(tweets$soggettivo~.,data=X0,method = "class",cp=0)
plotcp(modrp)
modrp=prune(modrp,cp = .003)
printcp(modrp)
plot(modrp)
text(modrp)
...
thr=quantile(predict(modrp)[,2],prop.table(table(tweets$soggettivo[-id_test]))[1])
thr
predetti=predict(modrp,newdata = X0[id_test,])[,2]>thr
tab=table( tweets$soggettivo[id_test],predetti)
tab

F_soggettivo=F_class(true = tweets$soggettivo[id_test] ,predetti==1)
F_obj=F_class(true = tweets$soggettivo[id_test]==0 ,predetti==0)
(F_obj+F_soggettivo)/2
```
...
```{r}
Xtest0=Xtest
Xtest0$prev_pos=predict(modlm_pos,newdata = Xtest)
Xtest0$prev_neg=predict(modlm_neg,newdata = Xtest)
Xtest0$prev_ironic=predict(modlm_ironic,newdata = Xtest)

contgg=grep("Conteggi\\.",names(Xtest0))
names(Xtest0)[contgg]=paste("simbolo",sep="",1:length(contgg))


yhat=predict(modrp,newdata = Xtest0)[,2]>thr
write.table(file='previsioni/predicted_task1_tree.txt',yhat,row.names = FALSE,col.names = FALSE)

load("/home/livio/bitbucket/r-bee/competitions/evalita2014_task1/evalita2014_task1.RData")

predetti=predict(modrp,newdata = Xtest0)[,2]>thr
tab=table( Ytest,predetti)
tab

F_soggettivo=F_class(true = Ytest ,predetti==1)
F_obj=F_class(true = Ytest==0 ,predetti==0)
(F_obj+F_soggettivo)/2
