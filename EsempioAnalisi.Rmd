...
creo il dataset di test per usarlo in seguito e ridugo `tweets` al solo training set:
...
write.table(file='predicted_task1.txt', yhat)
```
...
str(yhat)
write.table(file='predicted_task2.txt', yhat,row.names = FALSE,col.names = FALSE)
```
...
```{r}
yhat=predict(modlm,newdata = Xtest[,])>thr
write.table(file='predicted_task3.txt', yhat)
```
